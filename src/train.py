...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            'character encoding of input.txt, from '
    ...
    parser.add_argument(
        '--rnn_size', type=int, default=256, help='size of RNN hidden state')
    parser.add_argument(
        '--num_layers',
        type=int,
        default=2,
    ...
    parser.add_argument(
        '--model', type=str, default='lstm', help='rnn, gru, or lstm')
    parser.add_argument(
        '--batch_size', type=int, default=50, help='minibatch size')
    parser.add_argument(
        '--seq_length', type=int, default=25, help='RNN sequence length')
    parser.add_argument(
...
def train(args):
    data_loader = TextLoader(
        args.data_dir, args.batch_size, args.seq_length, args.input_encoding)
    ...
    if args.init_from is not None:
        ...
        assert os.path.isdir(args.init_from), (
            '{} must be a path'.format(args.init_from)
        )
        ...
        assert os.path.isfile(os.path.join(
            args.init_from, 'words_vocab.pkl')
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for e in range(model.epoch_pointer.eval(), args.num_epochs):
            ...
            for b in range(data_loader.pointer, data_loader.num_batches):
                ...
                if (
                    (e * data_loader.num_batches + b) % args.save_every == 0 or
                    (
